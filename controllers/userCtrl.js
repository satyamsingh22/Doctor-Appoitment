  ...
  try{
    ...
    notifcation.push({
      ...
      data:{
        ...
        oncClickPath: "/admin/doctrs"
      }
    })
    await userModel.findByIdAndUpdate(adminUser._id,{notifcation})
    res.status(201).send({
      success:true,
      message:"Doctor Account Applied Successfullt"
    })
...
const deleteAllNotificationController = async(req,res)=>{
  try{
    ...
    res.status(500).send({
      success:false,
      message:"unable to delete all notfication",
      error
    })
...
//get all doc call
const getAllDoctorsController = async (req, res) => { // Check if this function handles the request correctly
  try {
...
const userAppointmentsController = async (req, res) => {
  try {
    ...
    // Find appointments for the specified userId
    const appointment = await appointmentModel.find({ userId: req.body.userId });

    // Send success response with the fetched appointments
    res.status(200).send({
      success: true,
